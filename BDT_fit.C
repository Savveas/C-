...
TFile *file_TTbar = new TFile("output_TTbar.root");

int rbin=10;

TH1 *h_sig = (TH1*)file->Get("BDT_eval");
h_sig->Rebin(rbin);
TH1 *h_TTbar = (TH1*)file_TTbar->Get("BDT_eval_TTbar");
h_TTbar->Rebin(rbin);
...
RooAddPdf model_0("model_0", "Background", RooArgList(TTbar_pdf), RooArgList(Nexp_ttbar));
RooAddPdf model_1("model_1", "Signal + Background", RooArgList(sig_pdf, TTbar_pdf), RooArgList(Nexp_sig, Nexp_ttbar));


RooAddPdf model_0_th("model_0_th", "Background_th", RooArgList(TTbar_pdf), RooArgList(Nexp_ttbar_th ));
RooAddPdf model_1_th("model_1_th", "Signal + Background_th", RooArgList(sig_pdf, TTbar_pdf), RooArgList(Nexp_sig_th, Nexp_ttbar_th));


float Ntotal_B = ttbar_N;
float Ntotal_SB = sig_N+ttbar_N;
...
TCanvas *c_B_1 = new TCanvas("c_B_1", "Background Fit", 1400, 1000);
    ...
    model_1.plotOn(frame_B, LineColor(kRed),Components("TTbar_pdf"));
    frame_B1->Draw();
